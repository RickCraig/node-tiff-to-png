...
class TiffConverter {
  ...
  static count(converted, key, value) {
    ...
    let num = 0;
    for (const i in converted) if (converted[i][key] === value) num += 1;
    return num;
  }
  ...
  async convert(tiff, isArray, location) {
    ...
    const escapeSpaces = pathWithSpaces => {
      return pathWithSpaces.replace(/(\s+)/g, '\\$1');
    };
    ...
    try {
      ...
      error = { tiff, target, filename: targetPath, error: e };
    }

    const converted = { tiff, target, success, filename: targetPath };
