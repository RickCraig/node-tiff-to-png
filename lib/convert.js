 ...
 * saveInPlace: true, false or undefined (default)
 */
...
class TiffConverter {
  ...
  async convert(tiff, location) {
    ...
    if (this.options.tmpPath) {
      command += ` -define registry:temporary-path=${this.options.tmpPath}`;
    }

    const targetPath = `${target}/${prefix}%d${suffix}.${type}`;
    command += ` "${tiff}"`;
  ...
  async convertOne(tiff, location) {
    ...
    if (!location) return logger.error('The location folder is required');

    const total = 1;
    const { converted, error } = await this.convert(tiff, location);
